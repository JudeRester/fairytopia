				...
				<div id="threads_main" class="">
					...
					<div id="threads_left">
						<c:forEach items="${talkroom }" var="t">
							<div data-lobc="0" id="${t.talk_id }" class="pm_thread"
								title="Click to open conversation">
								<div class="th_wrap">
									<div class="th_user_thumb">
										<a href="#"> <img
											src="/fairy/profile/${t.mem_a eq sessionScope.user.mem_id ? t.mem_b: t.mem_a }/profile"
									...
									<a href="/note/notelist?talk_id=${t.talk_id }"
										class="th_snippet" onclick="return false;"> <!-- <img src="가장 최근 대화자 프로필 사진 위치"	height="30" width="30" class="thumb snippet_thumb"> -->
										${t.snippet }
									</a>
						...
						<div id="threads_right_bottom">
							<form action="/pm.php" method="post"
								...
								<input type="hidden" id="send_thread_id" name="send_thread_id"
									value="talk_id"> <input id="msg_submit" type="submit"
									value="Send"> <input name="csrf_token" type="hidden"
									value="토큰값">
							</form>
						</div>
					</div>
...
<script>
	...
	view_thread = function(talk_id, reload_messages) {
		var msg = $("#message"), tl = $("#threads_left"), thread = $("#"
				+ talk_id), tm = $("#threads_main");
		...
		ctw.show();
		var c = $("#threads_right");
		var err_div = $('<div style="color:red"></div>'), err_txt = "Error during processing, please reload page and try again.";
